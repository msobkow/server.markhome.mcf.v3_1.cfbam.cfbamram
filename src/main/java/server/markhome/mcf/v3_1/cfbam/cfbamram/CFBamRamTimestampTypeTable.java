...
// Description: Java 25 in-memory RAM DbIO implementation for TimestampType.
	...
	implements ICFBamTimestampTypeTable
{
	...
	private Map< ICFLibKeyHash256,
				CFBamBuffTimestampType > dictByPKey
		= new HashMap< ICFLibKeyHash256,
				CFBamBuffTimestampType >();
	private Map< CFBamBuffTimestampTypeBySchemaIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimestampType >> dictBySchemaIdx
		= new HashMap< CFBamBuffTimestampTypeBySchemaIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimestampType >>();
	...
	public ICFBamTimestampType createTimestampType( ICFSecAuthorization Authorization,
		ICFBamTimestampType iBuff )
	{
		final String S_ProcName = "createTimestampType";
		
		CFBamBuffTimestampType Buff = (CFBamBuffTimestampType)(schema.getTableTimestampDef().createTimestampDef( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamTimestampType.CLASS_CODE ) {
		...
		CFBamBuffTimestampTypeBySchemaIdxKey keySchemaIdx = (CFBamBuffTimestampTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimestampType > subdictSchemaIdx;
		if( dictBySchemaIdx.containsKey( keySchemaIdx ) ) {
			subdictSchemaIdx = dictBySchemaIdx.get( keySchemaIdx );
		}
		else {
			subdictSchemaIdx = new HashMap< ICFLibKeyHash256, CFBamBuffTimestampType >();
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTimestampType.CLASS_CODE) {
	...
	public ICFBamTimestampType lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTimestampType.lockDerived";
		ICFBamTimestampType buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamTimestampType[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTimestampType[] retList = new ICFBamTimestampType[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTimestampType > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTimestampType ) {
			return( (ICFBamTimestampType)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTimestampType ) ) {
					filteredList.add( (ICFBamTimestampType)buff );
				}
			}
			return( filteredList.toArray( new ICFBamTimestampType[0] ) );
		}
	}
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		CFBamBuffTimestampTypeBySchemaIdxKey key = (CFBamBuffTimestampTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampType().newBySchemaIdxKey();

		key.setRequiredSchemaDefId( SchemaDefId );
		ICFBamTimestampType[] recArray;
		if( dictBySchemaIdx.containsKey( key ) ) {
			Map< ICFLibKeyHash256, CFBamBuffTimestampType > subdictSchemaIdx
				= dictBySchemaIdx.get( key );
			recArray = new ICFBamTimestampType[ subdictSchemaIdx.size() ];
			Iterator< CFBamBuffTimestampType > iter = subdictSchemaIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffTimestampType > subdictSchemaIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffTimestampType >();
			dictBySchemaIdx.put( key, subdictSchemaIdx );
			recArray = new ICFBamTimestampType[0];
		}
		return( recArray );
	}
	...
	public ICFBamTimestampType readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTimestampType.readRec";
		ICFBamTimestampType buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTimestampType.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTimestampType.readAllRec";
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTimestampType.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamTimestampType buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamTimestampType buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamTimestampType)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	public ICFBamTimestampType[] readRecBySchemaIdx( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 SchemaDefId )
	{
		final String S_ProcName = "CFBamRamTimestampType.readRecBySchemaIdx() ";
		ICFBamTimestampType buff;
		ArrayList<ICFBamTimestampType> filteredList = new ArrayList<ICFBamTimestampType>();
		ICFBamTimestampType[] buffList = readDerivedBySchemaIdx( Authorization,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTimestampType.CLASS_CODE ) ) {
		...
		int revision )
	{
		...
		{
			return( (CFBamBuffTimestampType)cur );
		}
		...
		return( (CFBamBuffTimestampType)editCur );
	}
	...
	public ICFBamTimestampType updateTimestampType( ICFSecAuthorization Authorization,
		ICFBamTimestampType iBuff )
	{
		CFBamBuffTimestampType Buff = (CFBamBuffTimestampType)(schema.getTableTimestampDef().updateTimestampDef( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffTimestampType existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		CFBamBuffTimestampTypeBySchemaIdxKey existingKeySchemaIdx = (CFBamBuffTimestampTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampType().newBySchemaIdxKey();
		existingKeySchemaIdx.setRequiredSchemaDefId( existing.getRequiredSchemaDefId() );

		CFBamBuffTimestampTypeBySchemaIdxKey newKeySchemaIdx = (CFBamBuffTimestampTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimestampType > subdict;
		...
		if( dictBySchemaIdx.containsKey( newKeySchemaIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffTimestampType >();
		...
		ICFBamTimestampType iBuff )
	{
		...
		CFBamBuffTimestampType Buff = (CFBamBuffTimestampType)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTimestampType existing = dictByPKey.get( pkey );
		if( existing == null ) {
