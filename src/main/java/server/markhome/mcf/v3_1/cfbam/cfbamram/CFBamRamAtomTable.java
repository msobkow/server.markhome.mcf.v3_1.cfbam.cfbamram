	...
	implements ICFBamAtomTable
{
	...
	public ICFBamAtom lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamAtom.lockDerived";
		ICFBamAtom buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamAtom[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		Iterator< CFBamBuffAtom > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
	...
	public ICFBamAtom readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamAtom.readRec";
		...
		ICFBamAtom iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffAtom > subdict;
		...
		ICFBamAtom iBuff )
	{
		...
		CFBamBuffAtom Buff = (CFBamBuffAtom)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffAtom existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 argKey )
	{
		...
		CFBamBuffAtom cur;
		LinkedList<CFBamBuffAtom> matchSet = new LinkedList<CFBamBuffAtom>();
		Iterator<CFBamBuffAtom> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffAtom> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffAtom)(schema.getTableAtom().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
			int subClassCode = cur.getClassCode();
			if( ICFBamAtom.CLASS_CODE == subClassCode ) {
