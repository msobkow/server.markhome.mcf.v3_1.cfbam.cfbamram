...
// Description: Java 25 in-memory RAM DbIO implementation for Id32Gen.
	...
	implements ICFBamId32GenTable
{
		...
		ICFBamId32Gen iBuff )
	{
		...
		CFBamBuffId32Gen Buff = (CFBamBuffId32Gen)(schema.getTableInt32Type().createInt32Type( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamId32Gen.CLASS_CODE ) {
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamId32Gen.CLASS_CODE) {
	...
	public ICFBamId32Gen lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamId32Gen.lockDerived";
		ICFBamId32Gen buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamId32Gen[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamId32Gen[] retList = new ICFBamId32Gen[ dictByPKey.values().size() ];
		Iterator< CFBamBuffId32Gen > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamId32Gen ) {
			return( (ICFBamId32Gen)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamId32Gen ) ) {
					filteredList.add( (ICFBamId32Gen)buff );
				}
			}
			return( filteredList.toArray( new ICFBamId32Gen[0] ) );
		}
	}
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		ICFBamInt32Type buffList[] = schema.getTableInt32Type().readDerivedBySchemaIdx( Authorization,
			SchemaDefId );
		if( buffList == null ) {
			return( null );
		}
	...
	public ICFBamId32Gen readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamId32Gen.readRec";
		ICFBamId32Gen buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamId32Gen.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamId32Gen.readAllRec";
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamId32Gen.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamId32Gen buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamId32Gen buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamId32Gen)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		ICFBamId32Gen buff;
		ArrayList<ICFBamId32Gen> filteredList = new ArrayList<ICFBamId32Gen>();
		ICFBamId32Gen[] buffList = readDerivedBySchemaIdx( Authorization,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		int revision )
	{
		...
		return( (CFBamBuffId32Gen)editCur );
	}
		...
		ICFBamId32Gen iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffId32Gen > subdict;
		...
		ICFBamId32Gen iBuff )
	{
		...
		CFBamBuffId32Gen Buff = (CFBamBuffId32Gen)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffId32Gen existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffId32Gen cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffId32Gen> matchSet = new LinkedList<CFBamBuffId32Gen>();
		Iterator<CFBamBuffId32Gen> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffId32Gen> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffId32Gen)(schema.getTableId32Gen().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
