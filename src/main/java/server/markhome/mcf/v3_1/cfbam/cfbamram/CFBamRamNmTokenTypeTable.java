	...
	implements ICFBamNmTokenTypeTable
{
		...
		ICFBamNmTokenType iBuff )
	{
		...
		CFBamBuffNmTokenTypeBySchemaIdxKey keySchemaIdx = (CFBamBuffNmTokenTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryNmTokenType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffNmTokenType > subdictSchemaIdx;
		if( dictBySchemaIdx.containsKey( keySchemaIdx ) ) {
			subdictSchemaIdx = dictBySchemaIdx.get( keySchemaIdx );
		}
		else {
			subdictSchemaIdx = new HashMap< ICFLibKeyHash256, CFBamBuffNmTokenType >();
	...
	public ICFBamNmTokenType lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamNmTokenType.lockDerived";
		ICFBamNmTokenType buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamNmTokenType[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		Iterator< CFBamBuffNmTokenType > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		CFBamBuffNmTokenTypeBySchemaIdxKey key = (CFBamBuffNmTokenTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryNmTokenType().newBySchemaIdxKey();
		...
		if( dictBySchemaIdx.containsKey( key ) ) {
			...
			Iterator< CFBamBuffNmTokenType > iter = subdictSchemaIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffNmTokenType > subdictSchemaIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffNmTokenType >();
		...
		int revision )
	{
		...
		{
			return( (CFBamBuffNmTokenType)cur );
		}
		...
		return( (CFBamBuffNmTokenType)editCur );
	}
	...
	public ICFBamNmTokenType updateNmTokenType( ICFSecAuthorization Authorization,
		ICFBamNmTokenType iBuff )
	{
		CFBamBuffNmTokenType Buff = (CFBamBuffNmTokenType)(schema.getTableNmTokenDef().updateNmTokenDef( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffNmTokenType existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		CFBamBuffNmTokenTypeBySchemaIdxKey existingKeySchemaIdx = (CFBamBuffNmTokenTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryNmTokenType().newBySchemaIdxKey();
		existingKeySchemaIdx.setRequiredSchemaDefId( existing.getRequiredSchemaDefId() );

		CFBamBuffNmTokenTypeBySchemaIdxKey newKeySchemaIdx = (CFBamBuffNmTokenTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryNmTokenType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffNmTokenType > subdict;
		...
		if( dictBySchemaIdx.containsKey( newKeySchemaIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffNmTokenType >();
		...
		ICFBamNmTokenType iBuff )
	{
		...
		CFBamBuffNmTokenType Buff = (CFBamBuffNmTokenType)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffNmTokenType existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffNmTokenType cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffNmTokenType> matchSet = new LinkedList<CFBamBuffNmTokenType>();
		Iterator<CFBamBuffNmTokenType> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffNmTokenType> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffNmTokenType)(schema.getTableNmTokenType().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
