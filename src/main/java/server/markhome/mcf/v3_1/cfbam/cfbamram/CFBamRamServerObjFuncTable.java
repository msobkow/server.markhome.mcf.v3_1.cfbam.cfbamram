	...
	implements ICFBamServerObjFuncTable
{
		...
		ICFBamServerObjFunc iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffServerObjFunc > subdictRetTblIdx;
		if( dictByRetTblIdx.containsKey( keyRetTblIdx ) ) {
			subdictRetTblIdx = dictByRetTblIdx.get( keyRetTblIdx );
		}
		else {
			subdictRetTblIdx = new HashMap< ICFLibKeyHash256, CFBamBuffServerObjFunc >();
		...
		ICFLibKeyHash256 RetTableId )
	{
		...
		if( dictByRetTblIdx.containsKey( key ) ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffServerObjFunc > subdictRetTblIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffServerObjFunc >();
		...
		ICFBamPubSchema.CodeVisibilityEnum CodeVis )
	{
		...
		ICFBamServerObjFunc buff;
		ArrayList<ICFBamServerObjFunc> filteredList = new ArrayList<ICFBamServerObjFunc>();
		ICFBamServerObjFunc[] buffList = readDerivedByMethCodeVisIdx( Authorization,
			CodeVis );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamServerMethod.CLASS_CODE ) ) {
		...
		ICFBamPubSchema.CodeVisibilityEnum CodeVis )
	{
		...
		ICFBamServerObjFunc buff;
		ArrayList<ICFBamServerObjFunc> filteredList = new ArrayList<ICFBamServerObjFunc>();
		ICFBamServerObjFunc[] buffList = readDerivedByMethTableVisIdx( Authorization,
			TableId,
			CodeVis );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFBamServerObjFunc iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffServerObjFunc > subdict;
		...
		if( dictByRetTblIdx.containsKey( newKeyRetTblIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffServerObjFunc >();
