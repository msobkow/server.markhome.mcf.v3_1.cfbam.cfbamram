...
// Description: Java 25 in-memory RAM DbIO implementation for EnumDef.
	...
	implements ICFBamEnumDefTable
{
		...
		ICFBamEnumDef iBuff )
	{
		...
		CFBamBuffEnumDef Buff = (CFBamBuffEnumDef)(schema.getTableInt16Def().createInt16Def( Authorization,
			iBuff ));
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamEnumDef.CLASS_CODE) {
	...
	public ICFBamEnumDef[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		Iterator< CFBamBuffEnumDef > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
	...
	public ICFBamEnumDef readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamEnumDef.readRec";
		ICFBamEnumDef buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamEnumDef.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
		...
		int revision )
	{
		...
		return( (CFBamBuffEnumDef)editCur );
	}
	...
	public ICFBamEnumDef updateEnumDef( ICFSecAuthorization Authorization,
		ICFBamEnumDef iBuff )
	{
		CFBamBuffEnumDef Buff = (CFBamBuffEnumDef)(schema.getTableInt16Def().updateInt16Def( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffEnumDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		Map< ICFLibKeyHash256, CFBamBuffEnumDef > subdict;
		...
		ICFBamEnumDef iBuff )
	{
		...
		CFBamBuffEnumDef Buff = (CFBamBuffEnumDef)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffEnumDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 argKey )
	{
		...
		CFBamBuffEnumDef cur;
		LinkedList<CFBamBuffEnumDef> matchSet = new LinkedList<CFBamBuffEnumDef>();
		Iterator<CFBamBuffEnumDef> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffEnumDef> iterMatch = matchSet.iterator();
