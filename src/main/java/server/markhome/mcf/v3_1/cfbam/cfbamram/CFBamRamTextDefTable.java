...
// Description: Java 25 in-memory RAM DbIO implementation for TextDef.
	...
	implements ICFBamTextDefTable
{
		...
		ICFBamTextDef iBuff )
	{
		...
		CFBamBuffTextDef Buff = (CFBamBuffTextDef)(schema.getTableAtom().createAtom( Authorization,
			iBuff ));
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTextDef.CLASS_CODE) {
	...
	public ICFBamTextDef[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTextDef[] retList = new ICFBamTextDef[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTextDef > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTextDef ) {
			return( (ICFBamTextDef)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTextDef ) ) {
					filteredList.add( (ICFBamTextDef)buff );
				}
			}
			return( filteredList.toArray( new ICFBamTextDef[0] ) );
		}
	}
	...
	public ICFBamTextDef readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTextDef.readRec";
		ICFBamTextDef buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTextDef.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTextDef.readAllRec";
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTextDef.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamTextDef buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamTextDef buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamTextDef)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTextDef buff;
		ArrayList<ICFBamTextDef> filteredList = new ArrayList<ICFBamTextDef>();
		ICFBamTextDef[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		int revision )
	{
		...
		return( (CFBamBuffTextDef)editCur );
	}
	...
	public ICFBamTextDef updateTextDef( ICFSecAuthorization Authorization,
		ICFBamTextDef iBuff )
	{
		CFBamBuffTextDef Buff = (CFBamBuffTextDef)(schema.getTableAtom().updateAtom( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffTextDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		Map< ICFLibKeyHash256, CFBamBuffTextDef > subdict;
		...
		ICFBamTextDef iBuff )
	{
		...
		CFBamBuffTextDef Buff = (CFBamBuffTextDef)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTextDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 argKey )
	{
		...
		CFBamBuffTextDef cur;
		LinkedList<CFBamBuffTextDef> matchSet = new LinkedList<CFBamBuffTextDef>();
		Iterator<CFBamBuffTextDef> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffTextDef> iterMatch = matchSet.iterator();
