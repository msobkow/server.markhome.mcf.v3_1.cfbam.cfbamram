...
// Description: Java 25 in-memory RAM DbIO implementation for TimeType.
	...
	implements ICFBamTimeTypeTable
{
	...
	private Map< ICFLibKeyHash256,
				CFBamBuffTimeType > dictByPKey
		= new HashMap< ICFLibKeyHash256,
				CFBamBuffTimeType >();
	private Map< CFBamBuffTimeTypeBySchemaIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimeType >> dictBySchemaIdx
		= new HashMap< CFBamBuffTimeTypeBySchemaIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimeType >>();
		...
		ICFBamTimeType iBuff )
	{
		...
		CFBamBuffTimeType Buff = (CFBamBuffTimeType)(schema.getTableTimeDef().createTimeDef( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamTimeType.CLASS_CODE ) {
		...
		CFBamBuffTimeTypeBySchemaIdxKey keySchemaIdx = (CFBamBuffTimeTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimeType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimeType > subdictSchemaIdx;
		if( dictBySchemaIdx.containsKey( keySchemaIdx ) ) {
			subdictSchemaIdx = dictBySchemaIdx.get( keySchemaIdx );
		}
		else {
			subdictSchemaIdx = new HashMap< ICFLibKeyHash256, CFBamBuffTimeType >();
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTimeType.CLASS_CODE) {
	...
	public ICFBamTimeType lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTimeType.lockDerived";
		ICFBamTimeType buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamTimeType[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTimeType[] retList = new ICFBamTimeType[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTimeType > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTimeType ) {
			return( (ICFBamTimeType)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTimeType ) ) {
					filteredList.add( (ICFBamTimeType)buff );
				}
			}
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		CFBamBuffTimeTypeBySchemaIdxKey key = (CFBamBuffTimeTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimeType().newBySchemaIdxKey();

		key.setRequiredSchemaDefId( SchemaDefId );
		ICFBamTimeType[] recArray;
		if( dictBySchemaIdx.containsKey( key ) ) {
			Map< ICFLibKeyHash256, CFBamBuffTimeType > subdictSchemaIdx
				= dictBySchemaIdx.get( key );
			recArray = new ICFBamTimeType[ subdictSchemaIdx.size() ];
			Iterator< CFBamBuffTimeType > iter = subdictSchemaIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffTimeType > subdictSchemaIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffTimeType >();
			dictBySchemaIdx.put( key, subdictSchemaIdx );
			recArray = new ICFBamTimeType[0];
		}
		return( recArray );
	}
		...
		int revision )
	{
		...
		return( (CFBamBuffTimeType)editCur );
	}
		...
		ICFBamTimeType iBuff )
	{
		...
		CFBamBuffTimeTypeBySchemaIdxKey existingKeySchemaIdx = (CFBamBuffTimeTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimeType().newBySchemaIdxKey();
		existingKeySchemaIdx.setRequiredSchemaDefId( existing.getRequiredSchemaDefId() );

		CFBamBuffTimeTypeBySchemaIdxKey newKeySchemaIdx = (CFBamBuffTimeTypeBySchemaIdxKey)schema.getCFBamBuffFactory().getFactoryTimeType().newBySchemaIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimeType > subdict;
		...
		if( dictBySchemaIdx.containsKey( newKeySchemaIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffTimeType >();
		...
		ICFBamTimeType iBuff )
	{
		...
		CFBamBuffTimeType Buff = (CFBamBuffTimeType)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTimeType existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffTimeType cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffTimeType> matchSet = new LinkedList<CFBamBuffTimeType>();
		Iterator<CFBamBuffTimeType> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffTimeType> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffTimeType)(schema.getTableTimeType().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
