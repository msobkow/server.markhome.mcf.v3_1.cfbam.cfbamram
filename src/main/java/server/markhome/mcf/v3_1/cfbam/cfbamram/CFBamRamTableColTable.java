...
// Description: Java 25 in-memory RAM DbIO implementation for TableCol.
	...
	implements ICFBamTableColTable
{
	...
	private Map< ICFLibKeyHash256,
				CFBamBuffTableCol > dictByPKey
		= new HashMap< ICFLibKeyHash256,
				CFBamBuffTableCol >();
	private Map< CFBamBuffTableColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTableCol >> dictByTableIdx
		= new HashMap< CFBamBuffTableColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTableCol >>();
	private Map< CFBamBuffTableColByDataIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTableCol >> dictByDataIdx
		= new HashMap< CFBamBuffTableColByDataIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTableCol >>();
		...
		ICFBamTableCol iBuff )
	{
		...
		CFBamBuffTableCol Buff = (CFBamBuffTableCol)(schema.getTableValue().createValue( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamTableCol.CLASS_CODE ) {
		...
		CFBamBuffTableColByTableIdxKey keyTableIdx = (CFBamBuffTableColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByTableIdxKey();
		keyTableIdx.setRequiredTableId( Buff.getRequiredTableId() );

		CFBamBuffTableColByDataIdxKey keyDataIdx = (CFBamBuffTableColByDataIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByDataIdxKey();
	...
	public ICFBamTableCol[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTableCol[] retList = new ICFBamTableCol[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTableCol > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTableCol ) {
			return( (ICFBamTableCol)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTableCol ) ) {
					filteredList.add( (ICFBamTableCol)buff );
				}
			}
			return( filteredList.toArray( new ICFBamTableCol[0] ) );
		}
	}
		...
		ICFLibKeyHash256 TableId )
	{
		...
		CFBamBuffTableColByTableIdxKey key = (CFBamBuffTableColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByTableIdxKey();
		...
		if( dictByTableIdx.containsKey( key ) ) {
			...
			recArray = new ICFBamTableCol[ subdictTableIdx.size() ];
			Iterator< CFBamBuffTableCol > iter = subdictTableIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffTableCol > subdictTableIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffTableCol >();
			dictByTableIdx.put( key, subdictTableIdx );
			recArray = new ICFBamTableCol[0];
		}
		return( recArray );
	}
		...
		ICFLibKeyHash256 DataId )
	{
		...
		CFBamBuffTableColByDataIdxKey key = (CFBamBuffTableColByDataIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByDataIdxKey();
		...
		if( dictByDataIdx.containsKey( key ) ) {
			...
			recArray = new ICFBamTableCol[ subdictDataIdx.size() ];
			Iterator< CFBamBuffTableCol > iter = subdictDataIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
				recArray[ idx++ ] = iter.next();
			}
	...
	public ICFBamTableCol readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTableCol.readRec";
		ICFBamTableCol buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTableCol.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTableCol.readAllRec";
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTableCol.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamTableCol buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamTableCol buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamTableCol)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	public ICFBamTableCol[] readRecByTableIdx( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 TableId )
	{
		final String S_ProcName = "CFBamRamTableCol.readRecByTableIdx() ";
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByTableIdx( Authorization,
			TableId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTableCol.CLASS_CODE ) ) {
	...
	public ICFBamTableCol[] readRecByDataIdx( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 DataId )
	{
		final String S_ProcName = "CFBamRamTableCol.readRecByDataIdx() ";
		ICFBamTableCol buff;
		ArrayList<ICFBamTableCol> filteredList = new ArrayList<ICFBamTableCol>();
		ICFBamTableCol[] buffList = readDerivedByDataIdx( Authorization,
			DataId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTableCol.CLASS_CODE ) ) {
		...
		int revision )
	{
		...
		return( (CFBamBuffTableCol)editCur );
	}
		...
		ICFBamTableCol iBuff )
	{
		...
		CFBamBuffTableColByTableIdxKey existingKeyTableIdx = (CFBamBuffTableColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByTableIdxKey();
		existingKeyTableIdx.setRequiredTableId( existing.getRequiredTableId() );

		CFBamBuffTableColByTableIdxKey newKeyTableIdx = (CFBamBuffTableColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByTableIdxKey();
		newKeyTableIdx.setRequiredTableId( Buff.getRequiredTableId() );

		CFBamBuffTableColByDataIdxKey existingKeyDataIdx = (CFBamBuffTableColByDataIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByDataIdxKey();
		existingKeyDataIdx.setOptionalDataId( existing.getOptionalDataId() );

		CFBamBuffTableColByDataIdxKey newKeyDataIdx = (CFBamBuffTableColByDataIdxKey)schema.getCFBamBuffFactory().getFactoryTableCol().newByDataIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTableCol > subdict;
		...
		if( dictByTableIdx.containsKey( newKeyTableIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffTableCol >();
		...
		ICFBamTableCol iBuff )
	{
		...
		CFBamBuffTableCol Buff = (CFBamBuffTableCol)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTableCol existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 varTableId = existing.getRequiredTableId();
	...
	{
		CFBamBuffTableCol cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffTableCol> matchSet = new LinkedList<CFBamBuffTableCol>();
		Iterator<CFBamBuffTableCol> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffTableCol> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffTableCol)(schema.getTableTableCol().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
