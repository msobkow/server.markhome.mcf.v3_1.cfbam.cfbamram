...
// Description: Java 25 in-memory RAM DbIO implementation for TZTimeCol.
	...
	implements ICFBamTZTimeColTable
{
	...
	private Map< ICFLibKeyHash256,
				CFBamBuffTZTimeCol > dictByPKey
		= new HashMap< ICFLibKeyHash256,
				CFBamBuffTZTimeCol >();
	private Map< CFBamBuffTZTimeColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTZTimeCol >> dictByTableIdx
		= new HashMap< CFBamBuffTZTimeColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTZTimeCol >>();
	...
	public ICFBamTZTimeCol createTZTimeCol( ICFSecAuthorization Authorization,
		ICFBamTZTimeCol iBuff )
	{
		final String S_ProcName = "createTZTimeCol";
		
		CFBamBuffTZTimeCol Buff = (CFBamBuffTZTimeCol)(schema.getTableTZTimeDef().createTZTimeDef( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamTZTimeCol.CLASS_CODE ) {
		...
		CFBamBuffTZTimeColByTableIdxKey keyTableIdx = (CFBamBuffTZTimeColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTZTimeCol().newByTableIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTZTimeCol > subdictTableIdx;
		if( dictByTableIdx.containsKey( keyTableIdx ) ) {
			subdictTableIdx = dictByTableIdx.get( keyTableIdx );
		}
		else {
			subdictTableIdx = new HashMap< ICFLibKeyHash256, CFBamBuffTZTimeCol >();
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTZTimeCol.CLASS_CODE) {
	...
	public ICFBamTZTimeCol[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTZTimeCol[] retList = new ICFBamTZTimeCol[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTZTimeCol > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTZTimeCol ) {
			return( (ICFBamTZTimeCol)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTZTimeCol ) ) {
					filteredList.add( (ICFBamTZTimeCol)buff );
				}
			}
			return( filteredList.toArray( new ICFBamTZTimeCol[0] ) );
		}
	}
		...
		ICFLibKeyHash256 TableId )
	{
		...
		CFBamBuffTZTimeColByTableIdxKey key = (CFBamBuffTZTimeColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTZTimeCol().newByTableIdxKey();
		...
		if( dictByTableIdx.containsKey( key ) ) {
			...
			recArray = new ICFBamTZTimeCol[ subdictTableIdx.size() ];
			Iterator< CFBamBuffTZTimeCol > iter = subdictTableIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffTZTimeCol > subdictTableIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffTZTimeCol >();
			dictByTableIdx.put( key, subdictTableIdx );
			recArray = new ICFBamTZTimeCol[0];
		}
		return( recArray );
	}
	...
	public ICFBamTZTimeCol readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTZTimeCol.readRec";
		ICFBamTZTimeCol buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTZTimeCol.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTZTimeCol.readAllRec";
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTZTimeCol.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamTZTimeCol buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamTZTimeCol buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamTZTimeCol)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	public ICFBamTZTimeCol[] readRecByTableIdx( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 TableId )
	{
		final String S_ProcName = "CFBamRamTZTimeCol.readRecByTableIdx() ";
		ICFBamTZTimeCol buff;
		ArrayList<ICFBamTZTimeCol> filteredList = new ArrayList<ICFBamTZTimeCol>();
		ICFBamTZTimeCol[] buffList = readDerivedByTableIdx( Authorization,
			TableId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTZTimeCol.CLASS_CODE ) ) {
		...
		int revision )
	{
		...
		return( (CFBamBuffTZTimeCol)editCur );
	}
	...
	public ICFBamTZTimeCol updateTZTimeCol( ICFSecAuthorization Authorization,
		ICFBamTZTimeCol iBuff )
	{
		CFBamBuffTZTimeCol Buff = (CFBamBuffTZTimeCol)(schema.getTableTZTimeDef().updateTZTimeDef( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffTZTimeCol existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		CFBamBuffTZTimeColByTableIdxKey existingKeyTableIdx = (CFBamBuffTZTimeColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTZTimeCol().newByTableIdxKey();
		existingKeyTableIdx.setRequiredTableId( existing.getRequiredTableId() );

		CFBamBuffTZTimeColByTableIdxKey newKeyTableIdx = (CFBamBuffTZTimeColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTZTimeCol().newByTableIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTZTimeCol > subdict;
		...
		if( dictByTableIdx.containsKey( newKeyTableIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffTZTimeCol >();
		...
		ICFBamTZTimeCol iBuff )
	{
		...
		CFBamBuffTZTimeCol Buff = (CFBamBuffTZTimeCol)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTZTimeCol existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffTZTimeCol cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffTZTimeCol> matchSet = new LinkedList<CFBamBuffTZTimeCol>();
		Iterator<CFBamBuffTZTimeCol> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffTZTimeCol> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffTZTimeCol)(schema.getTableTZTimeCol().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
