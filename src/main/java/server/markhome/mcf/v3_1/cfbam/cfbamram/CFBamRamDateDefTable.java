...
// Description: Java 25 in-memory RAM DbIO implementation for DateDef.
	...
	implements ICFBamDateDefTable
{
		...
		ICFBamDateDef iBuff )
	{
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamDateDef.CLASS_CODE) {
	...
	public ICFBamDateDef lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamDateDef.lockDerived";
		ICFBamDateDef buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamDateDef[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamDateDef[] retList = new ICFBamDateDef[ dictByPKey.values().size() ];
		Iterator< CFBamBuffDateDef > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamDateDef ) {
			return( (ICFBamDateDef)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamDateDef ) ) {
					filteredList.add( (ICFBamDateDef)buff );
				}
			}
		...
		int revision )
	{
		...
		return( (CFBamBuffDateDef)editCur );
	}
		...
		ICFBamDateDef iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffDateDef > subdict;
		...
		ICFBamDateDef iBuff )
	{
		...
		CFBamBuffDateDef Buff = (CFBamBuffDateDef)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffDateDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 argKey )
	{
		...
		while( iterMatch.hasNext() ) {
			...
			if( ICFBamDateDef.CLASS_CODE == subClassCode ) {
