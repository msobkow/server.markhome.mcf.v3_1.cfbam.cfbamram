...
// Description: Java 25 in-memory RAM DbIO implementation for TZTimeDef.
	...
	implements ICFBamTZTimeDefTable
{
		...
		ICFBamTZTimeDef iBuff )
	{
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTZTimeDef.CLASS_CODE) {
	...
	public ICFBamTZTimeDef[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		Iterator< CFBamBuffTZTimeDef > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
	...
	public ICFBamTZTimeDef readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTZTimeDef.readRec";
		ICFBamTZTimeDef buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTZTimeDef.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTZTimeDef.readAllRec";
		ICFBamTZTimeDef buff;
		ArrayList<ICFBamTZTimeDef> filteredList = new ArrayList<ICFBamTZTimeDef>();
		...
		int revision )
	{
		...
		return( (CFBamBuffTZTimeDef)editCur );
	}
		...
		ICFBamTZTimeDef iBuff )
	{
		...
		Map< ICFLibKeyHash256, CFBamBuffTZTimeDef > subdict;
		...
		ICFBamTZTimeDef iBuff )
	{
		...
		CFBamBuffTZTimeDef Buff = (CFBamBuffTZTimeDef)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTZTimeDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
