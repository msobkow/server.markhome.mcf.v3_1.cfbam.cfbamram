...
// Description: Java 25 in-memory RAM DbIO implementation for DoubleDef.
	...
	implements ICFBamDoubleDefTable
{
	...
	public ICFBamDoubleDef createDoubleDef( ICFSecAuthorization Authorization,
		ICFBamDoubleDef iBuff )
	{
		final String S_ProcName = "createDoubleDef";
		
		CFBamBuffDoubleDef Buff = (CFBamBuffDoubleDef)(schema.getTableAtom().createAtom( Authorization,
			iBuff ));
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamDoubleDef.CLASS_CODE) {
	...
	public ICFBamDoubleDef lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamDoubleDef.lockDerived";
		ICFBamDoubleDef buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamDoubleDef[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamDoubleDef[] retList = new ICFBamDoubleDef[ dictByPKey.values().size() ];
		Iterator< CFBamBuffDoubleDef > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamDoubleDef ) {
			return( (ICFBamDoubleDef)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamDoubleDef ) ) {
					filteredList.add( (ICFBamDoubleDef)buff );
				}
			}
			return( filteredList.toArray( new ICFBamDoubleDef[0] ) );
		}
	}
	...
	public ICFBamDoubleDef readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamDoubleDef.readRec";
		ICFBamDoubleDef buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamDoubleDef.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamDoubleDef.readAllRec";
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamDoubleDef.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamDoubleDef buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamDoubleDef buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamDoubleDef)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamDoubleDef buff;
		ArrayList<ICFBamDoubleDef> filteredList = new ArrayList<ICFBamDoubleDef>();
		ICFBamDoubleDef[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		int revision )
	{
		...
		return( (CFBamBuffDoubleDef)editCur );
	}
	...
	public ICFBamDoubleDef updateDoubleDef( ICFSecAuthorization Authorization,
		ICFBamDoubleDef iBuff )
	{
		CFBamBuffDoubleDef Buff = (CFBamBuffDoubleDef)(schema.getTableAtom().updateAtom( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffDoubleDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		Map< ICFLibKeyHash256, CFBamBuffDoubleDef > subdict;
		...
		ICFBamDoubleDef iBuff )
	{
		...
		CFBamBuffDoubleDef Buff = (CFBamBuffDoubleDef)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffDoubleDef existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		ICFLibKeyHash256 argKey )
	{
		...
		CFBamBuffDoubleDef cur;
		LinkedList<CFBamBuffDoubleDef> matchSet = new LinkedList<CFBamBuffDoubleDef>();
		Iterator<CFBamBuffDoubleDef> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffDoubleDef> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffDoubleDef)(schema.getTableDoubleDef().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
			int subClassCode = cur.getClassCode();
			if( ICFBamDoubleDef.CLASS_CODE == subClassCode ) {
