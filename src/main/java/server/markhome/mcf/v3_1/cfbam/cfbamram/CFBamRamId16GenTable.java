...
// Description: Java 25 in-memory RAM DbIO implementation for Id16Gen.
	...
	implements ICFBamId16GenTable
{
		...
		ICFBamId16Gen iBuff )
	{
		...
		CFBamBuffId16Gen Buff = (CFBamBuffId16Gen)(schema.getTableInt16Type().createInt16Type( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamId16Gen.CLASS_CODE ) {
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamId16Gen.CLASS_CODE) {
	...
	public ICFBamId16Gen lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamId16Gen.lockDerived";
		ICFBamId16Gen buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamId16Gen[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamId16Gen[] retList = new ICFBamId16Gen[ dictByPKey.values().size() ];
		Iterator< CFBamBuffId16Gen > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamId16Gen ) {
			return( (ICFBamId16Gen)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamId16Gen ) ) {
					filteredList.add( (ICFBamId16Gen)buff );
				}
			}
			return( filteredList.toArray( new ICFBamId16Gen[0] ) );
		}
	}
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		ICFBamInt16Type buffList[] = schema.getTableInt16Type().readDerivedBySchemaIdx( Authorization,
			SchemaDefId );
		if( buffList == null ) {
			return( null );
		}
	...
	public ICFBamId16Gen readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamId16Gen.readRec";
		ICFBamId16Gen buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamId16Gen.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamId16Gen.readAllRec";
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamId16Gen.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamId16Gen buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamId16Gen buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamId16Gen)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 SchemaDefId )
	{
		...
		ICFBamId16Gen buff;
		ArrayList<ICFBamId16Gen> filteredList = new ArrayList<ICFBamId16Gen>();
		ICFBamId16Gen[] buffList = readDerivedBySchemaIdx( Authorization,
			SchemaDefId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		int revision )
	{
		...
		return( (CFBamBuffId16Gen)editCur );
	}
	...
	public ICFBamId16Gen updateId16Gen( ICFSecAuthorization Authorization,
		ICFBamId16Gen iBuff )
	{
		CFBamBuffId16Gen Buff = (CFBamBuffId16Gen)(schema.getTableInt16Type().updateInt16Type( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffId16Gen existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		Map< ICFLibKeyHash256, CFBamBuffId16Gen > subdict;
		...
		ICFBamId16Gen iBuff )
	{
		...
		CFBamBuffId16Gen Buff = (CFBamBuffId16Gen)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffId16Gen existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffId16Gen cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffId16Gen> matchSet = new LinkedList<CFBamBuffId16Gen>();
		Iterator<CFBamBuffId16Gen> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffId16Gen> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffId16Gen)(schema.getTableId16Gen().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
