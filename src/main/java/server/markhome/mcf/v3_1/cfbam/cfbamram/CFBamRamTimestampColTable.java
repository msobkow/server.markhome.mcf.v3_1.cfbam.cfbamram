...
// Description: Java 25 in-memory RAM DbIO implementation for TimestampCol.
	...
	implements ICFBamTimestampColTable
{
	...
	private Map< ICFLibKeyHash256,
				CFBamBuffTimestampCol > dictByPKey
		= new HashMap< ICFLibKeyHash256,
				CFBamBuffTimestampCol >();
	private Map< CFBamBuffTimestampColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimestampCol >> dictByTableIdx
		= new HashMap< CFBamBuffTimestampColByTableIdxKey,
				Map< ICFLibKeyHash256,
					CFBamBuffTimestampCol >>();
	...
	public ICFBamTimestampCol createTimestampCol( ICFSecAuthorization Authorization,
		ICFBamTimestampCol iBuff )
	{
		final String S_ProcName = "createTimestampCol";
		
		CFBamBuffTimestampCol Buff = (CFBamBuffTimestampCol)(schema.getTableTimestampDef().createTimestampDef( Authorization,
			iBuff ));
		ICFBamValue tail = null;
		if( Buff.getClassCode() == ICFBamTimestampCol.CLASS_CODE ) {
		...
		CFBamBuffTimestampColByTableIdxKey keyTableIdx = (CFBamBuffTimestampColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampCol().newByTableIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimestampCol > subdictTableIdx;
		if( dictByTableIdx.containsKey( keyTableIdx ) ) {
			subdictTableIdx = dictByTableIdx.get( keyTableIdx );
		}
		else {
			subdictTableIdx = new HashMap< ICFLibKeyHash256, CFBamBuffTimestampCol >();
		...
		if (Buff == null) {
			...
			if (classCode == ICFBamTimestampCol.CLASS_CODE) {
	...
	public ICFBamTimestampCol lockDerived( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTimestampCol.lockDerived";
		ICFBamTimestampCol buff;
		if( dictByPKey.containsKey( PKey ) ) {
			buff = dictByPKey.get( PKey );
		}
	...
	public ICFBamTimestampCol[] readAllDerived( ICFSecAuthorization Authorization ) {
		...
		ICFBamTimestampCol[] retList = new ICFBamTimestampCol[ dictByPKey.values().size() ];
		Iterator< CFBamBuffTimestampCol > iter = dictByPKey.values().iterator();
		int idx = 0;
		while( iter.hasNext() ) {
		...
		String Name )
	{
		...
		else if( buff instanceof ICFBamTimestampCol ) {
			return( (ICFBamTimestampCol)buff );
		}
		else {
			return( null );
		}
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		if( buffList == null ) {
			...
			ICFBamValue buff;
			ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
			for( int idx = 0; idx < buffList.length; idx ++ ) {
				buff = buffList[idx];
				if( ( buff != null ) && ( buff instanceof ICFBamTimestampCol ) ) {
					filteredList.add( (ICFBamTimestampCol)buff );
				}
			}
			return( filteredList.toArray( new ICFBamTimestampCol[0] ) );
		}
	}
		...
		ICFLibKeyHash256 TableId )
	{
		...
		CFBamBuffTimestampColByTableIdxKey key = (CFBamBuffTimestampColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampCol().newByTableIdxKey();

		key.setRequiredTableId( TableId );
		ICFBamTimestampCol[] recArray;
		if( dictByTableIdx.containsKey( key ) ) {
			Map< ICFLibKeyHash256, CFBamBuffTimestampCol > subdictTableIdx
				= dictByTableIdx.get( key );
			recArray = new ICFBamTimestampCol[ subdictTableIdx.size() ];
			Iterator< CFBamBuffTimestampCol > iter = subdictTableIdx.values().iterator();
			int idx = 0;
			while( iter.hasNext() ) {
			...
			Map< ICFLibKeyHash256, CFBamBuffTimestampCol > subdictTableIdx
				= new HashMap< ICFLibKeyHash256, CFBamBuffTimestampCol >();
			dictByTableIdx.put( key, subdictTableIdx );
			recArray = new ICFBamTimestampCol[0];
		}
		return( recArray );
	}
	...
	public ICFBamTimestampCol readRec( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 PKey )
	{
		final String S_ProcName = "CFBamRamTimestampCol.readRec";
		ICFBamTimestampCol buff = readDerived( Authorization, PKey );
		if( ( buff != null ) && ( buff.getClassCode() != ICFBamTimestampCol.CLASS_CODE ) ) {
			buff = null;
		}
		return( buff );
	}
	...
	{
		final String S_ProcName = "CFBamRamTimestampCol.readAllRec";
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readAllDerived( Authorization );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTimestampCol.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 Id )
	{
		...
		ICFBamTimestampCol buff = readDerivedByIdIdx( Authorization,
			Id );
		...
		String Name )
	{
		...
		ICFBamTimestampCol buff = readDerivedByUNameIdx( Authorization,
			ScopeId,
			Name );
		if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
			return( (ICFBamTimestampCol)buff );
		...
		ICFLibKeyHash256 ScopeId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByScopeIdx( Authorization,
			ScopeId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 DefSchemaId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByDefSchemaIdx( Authorization,
			DefSchemaId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByPrevIdx( Authorization,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByNextIdx( Authorization,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamValue.CLASS_CODE ) ) {
		...
		ICFLibKeyHash256 PrevId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByContPrevIdx( Authorization,
			ScopeId,
			PrevId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
		...
		ICFLibKeyHash256 NextId )
	{
		...
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByContNextIdx( Authorization,
			ScopeId,
			NextId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
	...
	public ICFBamTimestampCol[] readRecByTableIdx( ICFSecAuthorization Authorization,
		ICFLibKeyHash256 TableId )
	{
		final String S_ProcName = "CFBamRamTimestampCol.readRecByTableIdx() ";
		ICFBamTimestampCol buff;
		ArrayList<ICFBamTimestampCol> filteredList = new ArrayList<ICFBamTimestampCol>();
		ICFBamTimestampCol[] buffList = readDerivedByTableIdx( Authorization,
			TableId );
		for( int idx = 0; idx < buffList.length; idx ++ ) {
			buff = buffList[idx];
			if( ( buff != null ) && ( buff.getClassCode() == ICFBamTimestampCol.CLASS_CODE ) ) {
		...
		int revision )
	{
		...
		{
			return( (CFBamBuffTimestampCol)cur );
		}
		...
		return( (CFBamBuffTimestampCol)editCur );
	}
	...
	public ICFBamTimestampCol updateTimestampCol( ICFSecAuthorization Authorization,
		ICFBamTimestampCol iBuff )
	{
		CFBamBuffTimestampCol Buff = (CFBamBuffTimestampCol)(schema.getTableTimestampDef().updateTimestampDef( Authorization,	iBuff ));
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)Buff.getPKey();
		CFBamBuffTimestampCol existing = dictByPKey.get( pkey );
		if( existing == null ) {
		...
		CFBamBuffTimestampColByTableIdxKey existingKeyTableIdx = (CFBamBuffTimestampColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampCol().newByTableIdxKey();
		existingKeyTableIdx.setRequiredTableId( existing.getRequiredTableId() );

		CFBamBuffTimestampColByTableIdxKey newKeyTableIdx = (CFBamBuffTimestampColByTableIdxKey)schema.getCFBamBuffFactory().getFactoryTimestampCol().newByTableIdxKey();
		...
		Map< ICFLibKeyHash256, CFBamBuffTimestampCol > subdict;
		...
		if( dictByTableIdx.containsKey( newKeyTableIdx ) ) {
			...
			subdict = new HashMap< ICFLibKeyHash256, CFBamBuffTimestampCol >();
		...
		ICFBamTimestampCol iBuff )
	{
		...
		CFBamBuffTimestampCol Buff = (CFBamBuffTimestampCol)ensureRec(iBuff);
		int classCode;
		ICFLibKeyHash256 pkey = (ICFLibKeyHash256)(Buff.getPKey());
		CFBamBuffTimestampCol existing = dictByPKey.get( pkey );
		if( existing == null ) {
	...
	{
		CFBamBuffTimestampCol cur;
		boolean anyNotNull = false;
		anyNotNull = true;
		if( ! anyNotNull ) {
			return;
		}
		LinkedList<CFBamBuffTimestampCol> matchSet = new LinkedList<CFBamBuffTimestampCol>();
		Iterator<CFBamBuffTimestampCol> values = dictByPKey.values().iterator();
		...
		Iterator<CFBamBuffTimestampCol> iterMatch = matchSet.iterator();
		while( iterMatch.hasNext() ) {
			cur = iterMatch.next();
			cur = (CFBamBuffTimestampCol)(schema.getTableTimestampCol().readDerivedByIdIdx( Authorization,
				cur.getRequiredId() ));
